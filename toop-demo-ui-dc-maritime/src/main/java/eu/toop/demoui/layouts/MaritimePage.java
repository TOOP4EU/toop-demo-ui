...
public class MaritimePage extends CustomLayout
{
  ...
  private transient Timer timeoutTimer = new Timer ();
  ...
  public MaritimePage (final BaseView view)
  {
    ...
    for (final EPredefinedDocumentTypeIdentifier e : EPredefinedDocumentTypeIdentifier.values ())
      if (e.getID ().contains ("::Request##") &&
          (e.getID ().contains ("shipcertificate-list") || e.getID ().contains ("crewcertificate-list")))
      {
        aDocTypes.add (e);
      }
    ...
    // addComponent (legalPersonCompanyNameField,
    // "legalPersonCompanyNameField");
  ...
  private Button getCertificateButton (final String uri)
  {
    final Button button = new Button (VaadinIcons.ARROW_CIRCLE_DOWN_O);
    button.addStyleName (ValoTheme.BUTTON_SMALL);
    button.addClickListener (e -> sendCertficateRequest (uri));
    return button;
  }

  private void sendCertficateRequest (final String uri)
  {
    ...
    try
    {
      ...
      if (documentTypeField.getValue ().toString ().contains ("SHIPCERTIFICATE"))
      {
        ...
        formValues = new Request (countryCodeField.getValue (),
                                  EPredefinedDocumentTypeIdentifier.REQUEST_SHIPCERTIFICATE);
      ...
      else
      {
        ...
        formValues = new Request (countryCodeField.getValue (),
                                  EPredefinedDocumentTypeIdentifier.REQUEST_CREWCERTIFICATE);
      }
      ...
      final TOOPRequestMaker makeRequest = new TOOPRequestMaker (formValues);
  ...
  class SendRequest implements Button.ClickListener
  {
    ...
    public void buttonClick (final Button.ClickEvent clickEvent)
    {
      ...
      try
      {
        ...
        final Request formValues = new Request (countryCodeField.getValue (), documentTypeField.getValue ());
  ...
  public void addDocumentCertificateList (final List <DocumentDataBean> docResponseList)
  {
    _onResponseReceived ();

    final Grid <DocumentDataBean> grid = new Grid <> ();
    grid.setItems (docResponseList);
    grid.addComponentColumn (DocumentDataBean -> {
      final Button certificateButton = getCertificateButton (DocumentDataBean.getDocumentURI ());
